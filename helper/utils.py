        ...
        for edge in edges:
            ...
            if (keypoints[edge[0]][2] > threshold and keypoints[edge[1]][2] > threshold):
                for p in range(keypoints.shape[0]):
                    # draw the keypoints
                    cv2.circle(image, (int(keypoints[p, 0]*scale_x),int(keypoints[p, 1]*scale_x)), 
                ...
                for ie, e in enumerate(edges):
                    ...
                    cv2.line(image, (int(keypoints[e, 0][0]*scale_x),int(keypoints[e, 1][0]*scale_y)),
                            (int(keypoints[e, 0][1]*scale_x),int(keypoints[e, 1][1]*scale_y)),
